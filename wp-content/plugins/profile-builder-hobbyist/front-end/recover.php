	...
	if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'recover_password' && wp_verify_nonce($_POST['password_recovery_nonce_field'],'verify_true_password_recovery') ) {
        ...
        if( $messageNo == '1' ) {
            ...
            if( !empty( $query[0] ) ){
                ...
                $recoveruserMailMessage1 = sprintf( __('Hi <b>%1$s</b>, <br/><br/> We received a request to reset your RTL CBS Operator Site password. <br/><br/> If you ignore this message, your password won\'t be changed. <br/><br/> <a href="'. esc_url( add_query_arg( array( 'loginName' => $requestedUserNicename, 'key' => $key ), wppb_curpageurl() ) ).'">'.esc_url( add_query_arg( array( 'loginName' => $requestedUserNicename, 'key' => $key ), wppb_curpageurl() ) ). '<center><img src="http://operator.rtlcbsasia.com/wp-content/uploads/2017/02/reset_button.png" /></center></a><br/><br/> If the button above does not work, please visit the following link:%2$s', 'profile-builder'), $display_username_email, '<a href="'.esc_url( add_query_arg( array( 'loginName' => $requestedUserNicename, 'key' => $key ), wppb_curpageurl() ) ).'">'.esc_url( add_query_arg( array( 'loginName' => $requestedUserNicename, 'key' => $key ), wppb_curpageurl() ) ).'</a>');
