...
PRICE;
    ...
    if($sales_price > 0)
        $price_html = "<sub><del>{$currency_sign}{$price_html}</del></sub> ".'<div itemprop="offers" itemscope itemtype="http://schema.org/Offer"><span itemprop="priceCurrency" content="".wpdmpp_currency_code().">'.$currency_sign.'</span><span itemprop="price" content="'.number_format($sales_price,2).'">'.number_format($sales_price,2,".","").'</span></div>';
    else
...
PRICE;
    ...
    $prices = apply_filters("wpdmpp_product_price",$prices, $post_id);
